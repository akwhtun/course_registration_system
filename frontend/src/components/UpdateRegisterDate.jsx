    ...
    const fetchDate = async () => {
      try {
        ...
        if (response.status === 200) {
            setNewUpdateDate(response.data)
          setLoading(false);
        }
      } catch (error) {
  ...
  const handleUpdateDate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/crsforcu/backend/routes/date_api.php?endpoint=update_date`,
        newUpdateDate
      );
      if (response.status === 200) {
        setMessage(response.data.message)
        navigate(`/date/allow?update=Register Date updated successfully`);
            ...
            <div className="gird-cols-1 mb-12 grid  gap-12 px-4 lg:grid-cols-2">
              ...
              <div className='-mt-10 w-full'>
                ...
                <div className="w-full mt-5">
                  <Input label="Start Date" type='datetime-local' name="start_date" defaultValue={newUpdateDate.start_date} onChange={handleChange} />
                </div>
                <div className="w-full mt-5">
                  <Input label="End Date" type='datetime-local' name="end_date" defaultValue={newUpdateDate.end_date} onChange={handleChange} />
                </div>

                <Button type="submit" disabled={updateLoading} className="mt-6" fullWidth onClick={handleUpdateDate}>
...
export default UpdateRegisterDate;
